   ...
   */
    function quote() {
      ...
      //    $x = round(($cube / ( $maxlength * $maxwidth)),2)  ;
          ...
          if($x > 152 ) {  //  maximum allowed
			$maxlength = 152 ;   // so we set our length to maximum allowed
      ...
      //    sort($var) ;
      //    if(($var[0] * 1) + ($var[1] * 1) + $var[2] > 160 ) {   if($debug == 1) { echo "Girth exceeded1: $shipping_num_boxes " ; print_r($var) ;}
      ...
      if (in_array($country_id, $this->yamato_countries_nbr)) {
          ...
          if ( (MODULE_SHIPPING_YAMATO_FREE_SHIPPING != 'True') || ((int)$order->info['subtotal'] < (int)MODULE_SHIPPING_YAMATO_OVER) ) {
              ...
              if (isset($tmpQuote['error'])) {
                  ...
                  $this->quotes['module'] = $this->title . ' (' . $shipping_num_boxes . ' x ' . round($shipping_weight / $shipping_num_boxes,3) . 'kg, '. $ssize . 'cm)';
          ...
          if (!isset($tmpQuote['error'])) {
              ...
              $tmpQuote['option'] = TEXT_TIME_SPECIFY . zen_draw_pull_down_menu('yamato_timespec', $a_yamato_time, $timespec,'style="width: 115px;"');
...
*/
   ...
   */
  function keys() {
    return array('MODULE_SHIPPING_YAMATO_STATUS', 'MODULE_SHIPPING_YAMATO_HANDLING','MODULE_SHIPPING_YAMATO_FREE_SHIPPING', 'MODULE_SHIPPING_YAMATO_OVER', 'MODULE_SHIPPING_YAMATO_ZONE', 'MODULE_SHIPPING_YAMATO_SORT_ORDER');
  }

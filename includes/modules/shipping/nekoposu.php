...
*/
      ...
      //    $x = round(($cube / ( $maxlength * $maxwidth)),2)  ;
          ...
          if($x > 2.5 && $minheight <= 2.5 && $minlength <= 29 ) {  //  maximum allowed
          $maxlength = 29 ;   // so we set our length to maximum allowed
      ...
      // disable if too big 
      if (IS_ADMIN_FLAG == false && ($slength > 28 || $swidth > 11 || $sheight > 2.4 || ($slength+$swidth+$sheight > 48) || $shipping_weight > 1)) { 
          $this->enabled = false;
      }
...
*/
    ...
    function keys() {
      return array('MODULE_SHIPPING_NEKOPOSU_STATUS', 'MODULE_SHIPPING_NEKOPOSU_COST', 'MODULE_SHIPPING_NEKOPOSU_TAX_CLASS', 'MODULE_SHIPPING_NEKOPOSU_TAX_BASIS', 'MODULE_SHIPPING_NEKOPOSU_ZONE', 'MODULE_SHIPPING_NEKOPOSU_SORT_ORDER');
    }
  }

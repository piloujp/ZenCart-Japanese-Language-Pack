...
// Admin configuration
    ...
    'CFGTITLE_APT_OPTIMIZE_STATS' => '最後の最適化（自動管理）',
    'CFGDESC_APT_OPTIMIZE_STATS' => '直近のペアテーブル最適化（ディスク領域の再利用）の記録。ツール＞同時購入「ターボ版」に表示されます。自動的に管理されるため、手動で編集しないでください。形式: <code>日時|サイズ(バイト)|再利用サイズ(バイト)</code>。',

...
{
  $extendedOptions = array('doJsonProgressLogging'=>TRUE, 'doJsonProgressLoggingFileName'=>DEBUG_LOG_FOLDER . '/progress.json', 'id'=>'main', 'message'=>TEXT_LOADING_JAPANESE_ADMIN);
  $file = DIR_FS_INSTALL . 'sql/adminjp/mysql_japanese_admin_menu.sql';
  logDetails('processing file ' . $file);
  $error = $dbInstaller->parseSqlFile($file, $extendedOptions);
  // copying Japanese admin files
  $zb = new ZipArchive;
  if ($zb->open('admin_JP/adminjp.zip') === TRUE) {
    $zb->extractTo('../admin');
    $zb->close();
  }
}

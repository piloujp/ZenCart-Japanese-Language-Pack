...
class nekoposu  extends ZenShipping
{
    ...
    **/
    ...
    public function __construct()
    {
        ...
        $this->sort_order = zen_config('MODULE_SHIPPING_NEKOPOSU_SORT_ORDER');
        ...
        $this->tax_class = zen_config('MODULE_SHIPPING_NEKOPOSU_TAX_CLASS');
        $this->tax_basis = zen_config('MODULE_SHIPPING_NEKOPOSU_TAX_BASIS');

        // disable only when entire cart is free shipping
        if (zen_get_shipping_enabled($this->code)) {
            $this->enabled = zen_config('MODULE_SHIPPING_NEKOPOSU_STATUS') === 'True';
        } else {
            $this->enabled = false;
        }
    ...
    **/
    public function update_status()
    {
        ...
        if (!empty($box_sizes_array)) {
            ...
            // disable if too big
            if (IS_ADMIN_FLAG == false && ($box_sizes_array[0][0] > zen_config('MODULE_SHIPPING_NEKOPOSU_MAX_LENGTH') || $box_sizes_array[0][1] > zen_config('MODULE_SHIPPING_NEKOPOSU_MAX_WIDTH') || $box_sizes_array[0][2] > zen_config('MODULE_SHIPPING_NEKOPOSU_MAX_HEIGHT') || $girth > zen_config('MODULE_SHIPPING_NEKOPOSU_MAX_GIRTH') || $shipping_weight > zen_config('MODULE_SHIPPING_NEKOPOSU_MAX_WEIGHT'))) {
                $this->enabled = false;
            }
        }

        $this->checkEnabledForZone(zen_config('MODULE_SHIPPING_NEKOPOSU_ZONE'));
    ...
    public function quote($method = ''): array
    {
        ...
        if ((int)$order->delivery['country']['id'] === $this->japan_id) {
            ...
            $this->quotes['error'] = MODULE_SHIPPING_NEKOPOSU_TEXT_NOTAVAILABLE;
        }
    ...
    public function check()
    {
        if (!isset($this->_check)) {
            $this->_check = (int)(zen_config('MODULE_SHIPPING_NEKOPOSU_STATUS') !== null);
        }
        return $this->_check;
    }

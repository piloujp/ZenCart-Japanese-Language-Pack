...
class yamatocompact extends ZenShipping
{
    ...
    **/
    public function __construct()
    {
        ...
        $this->sort_order = zen_config('MODULE_SHIPPING_YAMATOCOMPACT_SORT_ORDER');
        ...
        $this->tax_class = zen_config('MODULE_SHIPPING_YAMATOCOMPACT_TAX_CLASS');
        $this->tax_basis = zen_config('MODULE_SHIPPING_YAMATOCOMPACT_TAX_BASIS');
        // disable only when entire cart is free shipping
        if (zen_get_shipping_enabled($this->code)) {
            $this->enabled = zen_config('MODULE_SHIPPING_YAMATOCOMPACT_STATUS') === 'True';
            $this->envelope_enabled = zen_config('MODULE_SHIPPING_YAMATOCOMPACT_CONTAINER_STATUS') === 'Both' || zen_config('MODULE_SHIPPING_YAMATOCOMPACT_CONTAINER_STATUS') === 'Envelope';
            $this->box_enabled = zen_config('MODULE_SHIPPING_YAMATOCOMPACT_CONTAINER_STATUS') === 'Both' || zen_config('MODULE_SHIPPING_YAMATOCOMPACT_CONTAINER_STATUS') === 'Box';
        } else {
            $this->enabled = false;
        }
    ...
    **/
    public function update_status()
    {
        ...
        if (!empty($box_sizes_array)) {
            ...
            // disable if too big
            if ($this->envelope_enabled === true && ($box_sizes_array[0][0] > zen_config('MODULE_SHIPPING_YAMATOCOMPACT_ENVELOPE_MAX_LENGTH') || $box_sizes_array[0][1] > zen_config('MODULE_SHIPPING_YAMATOCOMPACT_ENVELOPE_MAX_WIDTH') ||
            $box_sizes_array[0][2] > zen_config('MODULE_SHIPPING_YAMATOCOMPACT_ENVELOPE_MAX_HEIGHT') || $girth > zen_config('MODULE_SHIPPING_YAMATOCOMPACT_ENVELOPE_MAX_GIRTH'))) {
                $this->envelope_enabled = false;
            }
            if ($this->box_enabled === true && ($box_sizes_array[0][0] > zen_config('MODULE_SHIPPING_YAMATOCOMPACT_BOX_MAX_LENGTH') || $box_sizes_array[0][1] > zen_config('MODULE_SHIPPING_YAMATOCOMPACT_BOX_MAX_WIDTH') ||
            $box_sizes_array[0][2] > zen_config('MODULE_SHIPPING_YAMATOCOMPACT_BOX_MAX_HEIGHT') || $girth > zen_config('MODULE_SHIPPING_YAMATOCOMPACT_BOX_MAX_GIRTH'))) {
        ...
        $this->checkEnabledForZone(zen_config('MODULE_SHIPPING_YAMATOCOMPACT_ZONE'));
    ...
    **/
    public function quote($method = ''): array
    {
        ...
        if ((int)$country_id === $this->japan_id) {
            ...
            if (zen_config('MODULE_SHIPPING_YAMATOCOMPACT_FREE_SHIPPING') !== 'True' || (int)$order->info['subtotal'] < (int)zen_config('MODULE_SHIPPING_YAMATOCOMPACT_OVER')) {
                $rate = new _Yamato($this->code, MODULE_SHIPPING_YAMATOCOMPACT_TEXT_WAY_NORMAL, zen_get_zone_code(zen_config('STORE_COUNTRY'), zen_config('STORE_ZONE'), 0), zen_config('STORE_COUNTRY'));
                ...
                // 手数料
                $tmpQuote['cost'] += zen_config('MODULE_SHIPPING_YAMATOCOMPACT_HANDLING');
            } else {
    ...
    **/
    public function check()
    {
        if (!isset($this->_check)) {
            $this->_check = (int)(zen_config('MODULE_SHIPPING_YAMATOCOMPACT_STATUS') !== null);
        }
        return $this->_check;
    }

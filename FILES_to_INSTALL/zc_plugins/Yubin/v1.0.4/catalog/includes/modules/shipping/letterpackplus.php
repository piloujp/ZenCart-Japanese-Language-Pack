...
class letterpackplus extends ZenShipping
{
    ...
    **/
    ...
    public function __construct()
    {
        ...
        $this->sort_order = zen_config('MODULE_SHIPPING_LETTERPACKPLUS_SORT_ORDER');
        ...
        $this->tax_class = zen_config('MODULE_SHIPPING_LETTERPACKPLUS_TAX_CLASS');
        $this->tax_basis = zen_config('MODULE_SHIPPING_LETTERPACKPLUS_TAX_BASIS');

        // disable only when entire cart is free shipping
        if (zen_get_shipping_enabled($this->code)) {
            $this->enabled = zen_config('MODULE_SHIPPING_LETTERPACKPLUS_STATUS') === 'True';
        } else {
            $this->enabled = false;
        }
    ...
    **/
    ...
    {
        global $shipping_weight, $multiboxes, $box_sizes_array, $max_shipping_weight, $max_size_array;

        if ($this->enabled === false || IS_ADMIN_FLAG === true) {
            return;
        }

        $multiboxes = zen_config('MODULE_SHIPPING_LETTERPACKPLUS_MULTIBOX');
        $max_shipping_weight = zen_config('MODULE_SHIPPING_LETTERPACKPLUS_MAX_WEIGHT');
        $max_size_array = ['Max_length' => zen_config('MODULE_SHIPPING_LETTERPACKPLUS_MAX_LENGTH'), 'Max_width' => zen_config('MODULE_SHIPPING_LETTERPACKPLUS_MAX_WIDTH'), 'Max_height' => zen_config('MODULE_SHIPPING_LETTERPACKPLUS_MAX_HEIGHT'), 'Max_girth' => zen_config('MODULE_SHIPPING_LETTERPACKPLUS_MAX_GIRTH')];
        if (!empty($box_sizes_array)) {
            $girth = $box_sizes_array[0][0] + $box_sizes_array[0][1] + $box_sizes_array[0][2];
            // disable if too big
            if (IS_ADMIN_FLAG == false && ((($box_sizes_array[0][0] > $max_size_array['Max_length'] || $box_sizes_array[0][1] > $max_size_array['Max_width'] || $box_sizes_array[0][2] > $max_size_array['Max_height'] || $girth > $max_size_array['Max_girth']) && $multiboxes != 'Size') || ($shipping_weight > $max_shipping_weight && $multiboxes != 'Size'))) {
                $this->enabled = false;
            }
        }

        $this->checkEnabledForZone(zen_config('MODULE_SHIPPING_LETTERPACKPLUS_ZONE'));
    ...
    public function quote($method = ''): array
    {
        ...
        $this->quotes = [
            ...
            'methods' => [
                [
                    ...
                    'cost' => zen_config('MODULE_SHIPPING_LETTERPACKPLUS_COST') * $shipping_num_boxes,
                ],
            ],
        ];
        if (!empty($this->icon)) {
        ...
        if ((int)$order->delivery['country']['id'] === $this->japan_id) {
            ...
            $this->quotes['error'] = MODULE_SHIPPING_LETTERPACKPLUS_TEXT_NOTAVAILABLE;
        }
    ...
    public function check()
    {
        if (!isset($this->_check)) {
            $this->_check = (int)(zen_config('MODULE_SHIPPING_LETTERPACKPLUS_STATUS') !== null);
        }
        return $this->_check;
    }
    ...
    public function install(): void
    {
        ...
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('Enable or Disable Letter Pack Plus shipping method for some categories', 'MODULE_SHIPPING_LETTERPACKPLUS_CATEGORIES', 'Disable', 'Do you want to enable or disable Letter Pack Plus shipping for some categories?', '6', '0', 'zen_cfg_select_option([\'Enable\', \'Disable\'], ', now())");
        ...
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('Enable or Disable Letter Pack Plus shipping method for some products', 'MODULE_SHIPPING_LETTERPACKPLUS_PRODUCTS', 'Disable', 'Do you want to enable or disable Letter Pack Plus shipping for some products?', '6', '0', 'zen_cfg_select_option([\'Enable\', \'Disable\'], ', now())");
        ...
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Enable multi-boxing for this Method', 'MODULE_SHIPPING_LETTERPACKPLUS_MULTIBOX', 'None', 'Do you want to add new parcels when limit is reached and on what basis? Options are:<br>None - No multi-boxing<br>Size - Add new boxes when size limit is reached', '6', '0', 'zen_cfg_select_option([\'None\', \'Size\'], ', now())");

...
class yupackchilled extends ZenShipping
{
    ...
    **/
    public function __construct()
    {
        ...
        $this->sort_order = zen_config('MODULE_SHIPPING_YUPACKCHILLED_SORT_ORDER');
        ...
        $this->tax_class = zen_config('MODULE_SHIPPING_YUPACKCHILLED_TAX_CLASS');
        $this->tax_basis = zen_config('MODULE_SHIPPING_YUPACKCHILLED_TAX_BASIS');
        // disable only when entire cart is free shipping
        if (zen_get_shipping_enabled($this->code)) {
            $this->enabled = zen_config('MODULE_SHIPPING_YUPACKCHILLED_STATUS') === 'True';
        } else {
            $this->enabled = false;
        }
    ...
    **/
    public function update_status()
    {
        ...
        $this->checkEnabledForZone(zen_config('MODULE_SHIPPING_YUPACKCHILLED_ZONE'));
    ...
    **/
    public function quote($method = ''): array
    {
        ...
        $max_shipping_weight = zen_config('MODULE_SHIPPING_YUPACKCHILLED_MAX_WEIGHT');
        ...
        if ((int)$country_id === $this->japan_id) {
            ...
            if (zen_config('MODULE_SHIPPING_YUPACKCHILLED_FREE_SHIPPING') !== 'True' || (int)$order->info['subtotal'] < (int)zen_config('MODULE_SHIPPING_YUPACKCHILLED_OVER')) {
                ...
                // 手数料
                $tmpQuote['cost'] += zen_config('MODULE_SHIPPING_YUPACKCHILLED_HANDLING');
            } else {
                $tmpQuote = ['id' => $this->code, 'title' => MODULE_SHIPPING_YUPACKCHILLED_TEXT_WAY_NORMAL, 'cost' => 0];
            }
    ...
    **/
    public function check()
    {
        if (!isset($this->_check)) {
            $this->_check = (int)(zen_config('MODULE_SHIPPING_YUPACKCHILLED_STATUS') !== null);
        }
        return $this->_check;
    }
    ...
    **/
    public function install(): void
    {
        ...
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('Enable or Disable Refrigerated Yu-Pack shipping method for some products', 'MODULE_SHIPPING_YUPACKCHILLED_PRODUCTS', 'Disable', 'Do you want to enable or disable Refrigerated Yu-Pack shipping for some products?', '6', '0', 'zen_cfg_select_option([\'Enable\', \'Disable\'], ', now())");
        ...
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Free shipping settings', 'MODULE_SHIPPING_YUPACKCHILLED_FREE_SHIPPING', 'False', 'Would you like to activate the free shipping setting?Select False to give priority to other modules [Shipping cost]-[Free options]...', '6', '2', 'zen_cfg_select_option([\'True\', \'False\'], ', now())");

...
class ScriptedInstaller extends ScriptedInstallBase
{
        ...
        private array $default_jpparcelsea_headers = ['SHIPPING_RATES_ZONE_WEIGHT', 'Z1', 'Z2', 'Z3', 'Z4', 'Z5'];
        private array $default_jpparcelsea = [ // 小包郵便物:国際小包、船便 08-2024 International Parcels Surface 2 - 3 months - tracking
        ...
        private array $new_yupack_pricerank = [ // ２０２６年１０月現在の全ての送料（契約無し）　Full tarafication (no contract) as of October 2026
            ...
            'N04' => [1210,1600,2080,2450,3010,3380,5360],
            'N05' => [1270,1580,1960,2290,2930,3300,4670],
            'N06' => [1470,1860,2330,2730,3280,3670,5670],
            'N07' => [1550,1880,2220,2570,3220,3610,4970],
            'N08' => [1600,1990,2480,2860,3430,3820,5870],
            'N09' => [1750,2080,2410,2750,3420,3800,5830],
            'N10' => [1760,2170,2670,3020,3160,4000,6800],
            'N11' => [1910,2240,2590,2920,3610,4000,6040],
            'N12' => [1930,2260,2740,3080,3670,4060,7130],
        ];
        ...
        private array $default_yupackchilled_headers = ['60', '80', '100', '120', '140', '150'];
        private array $default_yupackchilled_surcharge = [225, 360, 675, 675, 1330, 2100];
    ...
    protected function executeInstall()
    {
        ...
        if (!$sniffer->table_exists(TABLE_SHIPPING_RATES)) {
            $this->executeInstallerSql(
                "CREATE TABLE " . TABLE_SHIPPING_RATES . " (
                    ...
                    quote_header JSON DEFAULT NULL,
                    PRIMARY KEY (id),
        ...
        $this->executeInstallerSql(
            "INSERT IGNORE INTO " . TABLE_SHIPPING_RATES . "
                (module, method, imple_date, update_date, quote_zone, quote_header)
            VALUES
                ('Yubin', 'jpparcelair', '" . $this->imple_date . "', NOW(), '" . json_encode($this->default_jpparcelair) . "', '" . json_encode($this->default_jpparcelair_headers) . "'),
    ...
    {
        global $sniffer;
        if (!$sniffer->field_exists(TABLE_SHIPPING_RATES, 'quote_header')) {
            $this->executeInstallerSql(
                "ALTER TABLE " . TABLE_SHIPPING_RATES . "
                ADD COLUMN quote_header JSON DEFAULT NULL
                ;"
            );
        }
        $this->executeInstallerSql(
            ...
            ON DUPLICATE KEY UPDATE
                ...
                quote_header = newshippingrates.quote_header
            ;"
        );

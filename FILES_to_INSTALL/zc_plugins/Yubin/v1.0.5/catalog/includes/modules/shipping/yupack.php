...
class yupack extends ZenShipping
{
    ...
    **/
    public function __construct()
    {
        ...
        $this->sort_order = zen_config('MODULE_SHIPPING_YUPACK_SORT_ORDER');
        ...
        $this->tax_class = zen_config('MODULE_SHIPPING_YUPACK_TAX_CLASS');
        $this->tax_basis = zen_config('MODULE_SHIPPING_YUPACK_TAX_BASIS');
        // disable only when entire cart is free shipping
        if (zen_get_shipping_enabled($this->code)) {
            $this->enabled = zen_config('MODULE_SHIPPING_YUPACK_STATUS') === 'True';
        } else {
            $this->enabled = false;
        }
    ...
    **/
    public function update_status()
    {
        ...
        $this->checkEnabledForZone(zen_config('MODULE_SHIPPING_YUPACK_ZONE'));
    ...
    **/
    public function quote($method = ''): array
    {
        ...
        $max_shipping_weight = zen_config('MODULE_SHIPPING_YUPACK_MAX_WEIGHT');
        ...
        if ((int)$country_id === $this->japan_id) {
            ...
            if (zen_config('MODULE_SHIPPING_YUPACK_FREE_SHIPPING') !== 'True' || (int)$order->info['subtotal'] < (int)zen_config('MODULE_SHIPPING_YUPACK_OVER')) {
                $rate = new _Yupack($this->code, MODULE_SHIPPING_YUPACK_TEXT_WAY_NORMAL, zen_get_zone_code(zen_config('STORE_COUNTRY'), zen_config('STORE_ZONE'), 0), zen_config('STORE_COUNTRY'));
                ...
                // 手数料
                $tmpQuote['cost'] += zen_config('MODULE_SHIPPING_YUPACK_HANDLING');
            } else {
                $tmpQuote = ['id' => $this->code, 'title' => MODULE_SHIPPING_YUPACK_TEXT_WAY_NORMAL, 'cost' => 0];
            }
    ...
    **/
    public function check()
    {
        if (!isset($this->_check)) {
            $this->_check = (int)(zen_config('MODULE_SHIPPING_YUPACK_STATUS') !== null);
        }
        return $this->_check;
    }
    ...
    **/
    public function install(): void
    {
        ...
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('Enable or Disable Yu-Pack shipping method for some categories', 'MODULE_SHIPPING_YUPACK_CATEGORIES', 'Disable', 'Do you want to enable or disable Yu-Pack shipping for some categories?', '6', '0', 'zen_cfg_select_option([\'Enable\', \'Disable\'], ', now())");
        ...
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('Enable or Disable Yu-Pack shipping method for some products', 'MODULE_SHIPPING_YUPACK_PRODUCTS', 'Disable', 'Do you want to enable or disable Yu-Pack shipping for some products?', '6', '0', 'zen_cfg_select_option([\'Enable\', \'Disable\'], ', now())");

...
class jpparcelsea extends ZenShipping
{
    ...
    **/
    public function __construct()
    {
        ...
        $this->sort_order = zen_config('MODULE_SHIPPING_JPPARCELSEA_SORT_ORDER');
        if (null === $this->sort_order) {
            return;
        }

        // $Pversion = zen_get_plugin_version('Yubin');
        $this->icon = ''; // (!empty($Pversion)) ? HTTPS_SERVER . DIR_WS_CATALOG . 'zc_plugins/Yubin/' . $Pversion . '/catalog/includes/templates/default/images/icons/shipping_jpparcelsea.gif' : '';
        $this->tax_class = zen_config('MODULE_SHIPPING_JPPARCELSEA_TAX_CLASS');
        $this->tax_basis = zen_config('MODULE_SHIPPING_JPPARCELSEA_TAX_BASIS');
        // disable only when entire cart is free shipping
        if (zen_get_shipping_enabled($this->code)) {
            $this->enabled = zen_config('MODULE_SHIPPING_JPPARCELSEA_STATUS') === 'True';
        } else {
            $this->enabled = false;
        }
    ...
    **/
    public function update_status()
    {
        ...
        $this->checkEnabledForZone(zen_config('MODULE_SHIPPING_JPPARCELSEA_ZONE'));
        ...
        if ($this->enabled) {
            ...
            //
            $this->notify('NOTIFY_SHIPPING_JPPARCELSEA_UPDATE_STATUS', [], $this->enabled);
        }
    }
    /**
    ...
    **/
    public function quote($method = ''): array
    {
        ...
        $max_shipping_weight = zen_config('MODULE_SHIPPING_JPPARCELSEA_MAX_WEIGHT');
        if (zen_config('MODULE_SHIPPING_JPPARCELSEA_FREE_SHIPPING') !== 'True' || (int)$cart->show_total() < (int)zen_config('MODULE_SHIPPING_JPPARCELSEA_OVER')) {
            $rate = new _JpParcel($this->code, MODULE_SHIPPING_JPPARCELSEA_TEXT_WAY_NORMAL, $this->country_code);
            $multiboxes = zen_config('MODULE_SHIPPING_JPPARCELSEA_MULTIBOX');
            if (!empty($box_array)) {
                $total_boxes_quote = 0;
            ...
            // 手数料
            $tmpQuote['cost'] += zen_config('MODULE_SHIPPING_JPPARCELSEA_HANDLING');
        } else {
            $tmpQuote = ['id' => $this->code, 'title' => MODULE_SHIPPING_JPPARCELSEA_TEXT_WAY_NORMAL, 'cost' => 0];
        }
    ...
    **/
    public function check()
    {
        if (!isset($this->_check)) {
            $this->_check = (int)(zen_config('MODULE_SHIPPING_JPPARCELSEA_STATUS') !== null);
        }
        return $this->_check;
    }
    /**
    ...
    **/
    public function install(): void
    {
        ...
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('Enable or Disable Japan Sea Parcel shipping method for some categories', 'MODULE_SHIPPING_JPPARCELSEA_CATEGORIES', 'Disable', 'Do you want to enable or disable Japan Sea Parcel shipping for some categories?', '6', '0', 'zen_cfg_select_option([\'Enable\', \'Disable\'], ', now())");
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) VALUES ('Enable/Disable categories IDs list', 'MODULE_SHIPPING_JPPARCELSEA_CAT_LIST', '', 'Comma separated list of categoies IDs to be enabled or disabled, depending on above option.', '6', '0', now())");
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('Enable or Disable Japan Sea Parcel shipping method for some products', 'MODULE_SHIPPING_JPPARCELSEA_PRODUCTS', 'Disable', 'Do you want to enable or disable Japan Sea Parcel shipping for some products?', '6', '0', 'zen_cfg_select_option([\'Enable\', \'Disable\'], ', now())");
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) VALUES ('Enable/Disable products IDs list', 'MODULE_SHIPPING_JPPARCELSEA_PROD_LIST', '', 'Comma separated list of products IDs to be enables or disabled, depending on above option.', '6', '0', now())");
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Enable multi-boxing for this Method', 'MODULE_SHIPPING_JPPARCELSEA_MULTIBOX', 'None', 'Do you want to add new parcels when limit is reached and on what basis? Options are:<br>None - No multi-boxing<br>Weight - New boxes based on weight limit', '6', '0', 'zen_cfg_select_option([\'None\', \'Weight\'], ', now())");
        ...
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Free shipping settings', 'MODULE_SHIPPING_JPPARCELSEA_FREE_SHIPPING', 'False', 'Would you like to activate the free shipping setting?Select False to give priority to other modules [Shipping cost]-[Free options]...', '6', '2', 'zen_cfg_select_option([\'True\', \'False\'], ', now())");
    ...
    **/
    public function keys(): array
    {
        return [
            ...
            'MODULE_SHIPPING_JPPARCELSEA_CATEGORIES',
            'MODULE_SHIPPING_JPPARCELSEA_CAT_LIST',
            'MODULE_SHIPPING_JPPARCELSEA_PRODUCTS',
            'MODULE_SHIPPING_JPPARCELSEA_PROD_LIST',

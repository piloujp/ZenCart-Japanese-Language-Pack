...
class letterpacklite extends ZenShipping
{
    ...
    **/
    ...
    public function __construct()
    {
        ...
        $this->sort_order = zen_config('MODULE_SHIPPING_LETTERPACKLITE_SORT_ORDER');
        if (null === $this->sort_order) {
            return;
        }

        $Pversion = zen_get_plugin_version('Yubin');
        $this->icon = (!empty($Pversion)) ? HTTPS_SERVER . DIR_WS_CATALOG . 'zc_plugins/Yubin/' . $Pversion . '/catalog/includes/templates/default/images/icons/shipping_letterpacklite.gif' : '';
        $this->tax_class = zen_config('MODULE_SHIPPING_LETTERPACKLITE_TAX_CLASS');
        $this->tax_basis = zen_config('MODULE_SHIPPING_LETTERPACKLITE_TAX_BASIS');

        // disable only when entire cart is free shipping
        if (zen_get_shipping_enabled($this->code)) {
            $this->enabled = zen_config('MODULE_SHIPPING_LETTERPACKLITE_STATUS') === 'True';
        } else {
            $this->enabled = false;
        }
    ...
    **/
    ...
    {
        global $shipping_weight, $box_sizes_array;
        ...
        if (!empty($box_sizes_array)) {
            ...
            // disable if too big
            if (IS_ADMIN_FLAG == false && ($box_sizes_array[0][0] > zen_config('MODULE_SHIPPING_LETTERPACKLITE_MAX_LENGTH') || $box_sizes_array[0][1] > zen_config('MODULE_SHIPPING_LETTERPACKLITE_MAX_WIDTH') || $box_sizes_array[0][2] > zen_config('MODULE_SHIPPING_LETTERPACKLITE_MAX_HEIGHT') || $girth > zen_config('MODULE_SHIPPING_LETTERPACKLITE_MAX_GIRTH') || $shipping_weight > zen_config('MODULE_SHIPPING_LETTERPACKLITE_MAX_WEIGHT'))) {
                $this->enabled = false;
            }
        }

        $this->checkEnabledForZone(zen_config('MODULE_SHIPPING_LETTERPACKLITE_ZONE'));
    ...
    public function quote($method = ''): array
    {
        ...
        $this->quotes = [
            ...
            'methods' => [
                [
                    ...
                    'cost' => zen_config('MODULE_SHIPPING_LETTERPACKLITE_COST'),
                ],
            ],
        ];
        if (!empty($this->icon)) {
        ...
        if ((int)$order->delivery['country']['id'] === $this->japan_id) {
            if ($this->tax_class > 0) {
            ...
            $this->quotes['error'] = MODULE_SHIPPING_LETTERPACKLITE_TEXT_NOTAVAILABLE;
        }
    ...
    public function check()
    {
        if (!isset($this->_check)) {
            $this->_check = (int)(zen_config('MODULE_SHIPPING_LETTERPACKLITE_STATUS') !== null);
        }
        return $this->_check;
    }
    ...
    public function install(): void
    {
        ...
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('Enable or Disable Letter Pack Lite shipping method for some categories', 'MODULE_SHIPPING_LETTERPACKLITE_CATEGORIES', 'Disable', 'Do you want to enable or disable Letter Pack Lite shipping for some categories?', '6', '0', 'zen_cfg_select_option([\'Enable\', \'Disable\'], ', now())");
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) VALUES ('Enable/Disable categories IDs list', 'MODULE_SHIPPING_LETTERPACKLITE_CAT_LIST', '', 'Comma separated list of categoies IDs to be enabled or disabled, depending on above option.', '6', '0', now())");
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('Enable or Disable Letter Pack Lite shipping method for some products', 'MODULE_SHIPPING_LETTERPACKLITE_PRODUCTS', 'Disable', 'Do you want to enable or disable Letter Pack Lite shipping for some products?', '6', '0', 'zen_cfg_select_option([\'Enable\', \'Disable\'], ', now())");
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) VALUES ('Enable/Disable products IDs list', 'MODULE_SHIPPING_LETTERPACKLITE_PROD_LIST', '', 'Comma separated list of products IDs to be enables or disabled, depending on above option.', '6', '0', now())");
    ...
    {
        return [
            'MODULE_SHIPPING_LETTERPACKLITE_STATUS',
            'MODULE_SHIPPING_LETTERPACKLITE_CATEGORIES',
            'MODULE_SHIPPING_LETTERPACKLITE_CAT_LIST',
            'MODULE_SHIPPING_LETTERPACKLITE_PRODUCTS',
            'MODULE_SHIPPING_LETTERPACKLITE_PROD_LIST',
            'MODULE_SHIPPING_LETTERPACKLITE_COST',
            'MODULE_SHIPPING_LETTERPACKLITE_MAX_WEIGHT',
            'MODULE_SHIPPING_LETTERPACKLITE_MAX_LENGTH',
            'MODULE_SHIPPING_LETTERPACKLITE_MAX_WIDTH',
            'MODULE_SHIPPING_LETTERPACKLITE_MAX_HEIGHT',
            'MODULE_SHIPPING_LETTERPACKLITE_MAX_GIRTH',
            'MODULE_SHIPPING_LETTERPACKLITE_TAX_CLASS',
            'MODULE_SHIPPING_LETTERPACKLITE_TAX_BASIS',
            'MODULE_SHIPPING_LETTERPACKLITE_ZONE',

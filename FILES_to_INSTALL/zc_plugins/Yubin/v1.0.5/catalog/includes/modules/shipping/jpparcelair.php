...
class jpparcelair extends ZenShipping
{
    /**
     * $country_code is country ISO 2 letters code International Parcels Air is shipping to
    **/
    ...
    **/
    public function __construct()
    {
        ...
        $this->sort_order = zen_config('MODULE_SHIPPING_JPPARCELAIR_SORT_ORDER');
        if (null === $this->sort_order) {
            return;
        }

        // $Pversion = zen_get_plugin_version('Yubin');
        $this->icon = ''; // (!empty($Pversion)) ? HTTPS_SERVER . DIR_WS_CATALOG . 'zc_plugins/Yubin/' . $Pversion . '/catalog/includes/templates/default/images/icons/shipping_jpparcelair.gif' : '';
        $this->tax_class = zen_config('MODULE_SHIPPING_JPPARCELAIR_TAX_CLASS');
        $this->tax_basis = zen_config('MODULE_SHIPPING_JPPARCELAIR_TAX_BASIS');
        // disable only when entire cart is free shipping
        if (zen_get_shipping_enabled($this->code)) {
            $this->enabled = zen_config('MODULE_SHIPPING_JPPARCELAIR_STATUS') === 'True';
        } else {
            $this->enabled = false;
        }
    ...
    **/
    public function update_status()
    {
        ...
        $this->checkEnabledForZone(zen_config('MODULE_SHIPPING_JPPARCELAIR_ZONE'));
        ...
        if ($this->enabled) {
            ...
            //
            $this->notify('NOTIFY_SHIPPING_JPPARCELAIR_UPDATE_STATUS', [], $this->enabled);
        }
    }
    /**
    ...
    **/
    public function quote($method = ''): array
    {
        ...
        $max_shipping_weight = zen_config('MODULE_SHIPPING_JPPARCELAIR_MAX_WEIGHT');
        if (zen_config('MODULE_SHIPPING_JPPARCELAIR_FREE_SHIPPING') !== 'True' || (int)$cart->show_total() < (int)zen_config('MODULE_SHIPPING_JPPARCELAIR_OVER')) {
            $rate = new _JpParcel($this->code, MODULE_SHIPPING_JPPARCELAIR_TEXT_WAY_NORMAL, $this->country_code);
            $multiboxes = zen_config('MODULE_SHIPPING_JPPARCELAIR_MULTIBOX');
            if (!empty($box_array)) {
                $total_boxes_quote = 0;
            ...
            // 手数料
            $tmpQuote['cost'] += zen_config('MODULE_SHIPPING_JPPARCELAIR_HANDLING');
        } else {
            $tmpQuote = ['id' => $this->code, 'title' => MODULE_SHIPPING_JPPARCELAIR_TEXT_WAY_NORMAL, 'cost' => 0];
        }
    ...
    **/
    public function check()
    {
        if (!isset($this->_check)) {
            $this->_check = (int)(zen_config('MODULE_SHIPPING_JPPARCELAIR_STATUS') !== null);
        }
        return $this->_check;
    }
    /**
    ...
    **/
    public function install(): void
    {
        ...
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Enable multi-boxing for this Method', 'MODULE_SHIPPING_JPPARCELAIR_MULTIBOX', 'None', 'Do you want to add new parcels when limit is reached and on what basis? Options are:<br>None - No multi-boxing<br>Weight - New boxes based on weight limit', '6', '0', 'zen_cfg_select_option([\'None\', \'Weight\'], ', now())");
    ...
    **/
    ...
    {
        return [
            'MODULE_SHIPPING_JPPARCELAIR_STATUS',
            'MODULE_SHIPPING_JPPARCELAIR_CATEGORIES',
            'MODULE_SHIPPING_JPPARCELAIR_CAT_LIST',
            'MODULE_SHIPPING_JPPARCELAIR_PRODUCTS',
            'MODULE_SHIPPING_JPPARCELAIR_PROD_LIST',
            'MODULE_SHIPPING_JPPARCELAIR_MULTIBOX',
            'MODULE_SHIPPING_JPPARCELAIR_HANDLING',

...
function zen_address_format($address_format_id = 1, $incoming = array(), $html = false, $boln = '', $eoln = "\n")
{
    ...
    $address['salutation'] = !empty($incoming['salutation']) ? zen_output_string_protected($incoming['salutation']) : (!empty($incoming['firstname']) || !empty($incoming['lastname']) || !empty($incoming['name']) ? ADDRESS_SALUTATION : '');

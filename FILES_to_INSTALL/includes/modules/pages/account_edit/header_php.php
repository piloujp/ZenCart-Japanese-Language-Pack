...
if (!empty($_POST['action']) && $_POST['action'] === 'process') {
    ...
    if ($_SESSION['language'] == 'japanese') {
        ...
        if (mb_strlen($lastname_kana) < zen_config('ENTRY_LAST_NAME_MIN_LENGTH')) {
            $error = true;
    ...
    if ($error === false) {
        ...
        if ($_SESSION['language'] == 'japanese') {
            $sql_data_array[] = ['fieldName'=>'customers_firstname_kana', 'value' => $firstname_kana, 'type' => 'stringIgnoreNull'];
            $sql_data_array[] = ['fieldName'=>'customers_lastname_kana', 'value' => $lastname_kana, 'type' => 'stringIgnoreNull'];
        }
